...
class NarBuilder(BuilderInterface):
    ...
    def install_dependencies(self, directory: Path, cache_dir: str) -> None:
        ...
        # De-quote any accidentally quoted requirement strings
        for i, arg in enumerate(install_arguments):
            if i >= 4:  # after "... python -m pip install"
                install_arguments[i] = arg.strip("'\"")

        # Optional: write argv so you can verify what pip sees
        try:
            Path("pip-args.log").write_text("\n".join(install_arguments) + "\n", encoding="utf-8")
        except Exception:
            Path("/tmp/pip-args.log").write_text("\n".join(install_arguments) + "\n", encoding="utf-8")        
